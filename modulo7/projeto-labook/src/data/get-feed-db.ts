...
export class GetFeedDatabase
  extends ConnectionDatabase
  implements GetFeedRepository
{
  async get(user_id: string): Promise<Post[]> {
    const posts: Post[] = await GetFeedDatabase.connection("labook_friendships")
      .select(
